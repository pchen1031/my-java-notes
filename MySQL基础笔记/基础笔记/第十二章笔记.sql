...
f1 FLOAT,
f2 FLOAT(5,2),#FLOAT(M,D)或DOUBLE(M,D)为非标准写法
f3 DOUBLE,
f4 DOUBLE(5,2)
);
...
SELECT NOW()#受time_zone 设置影响
FROM DUAL;

SELECT UNIX_TIMESTAMP();#不受 time_zone 设置的影响
...
);#意思是每次向test_enum这个表插入新数据的时候只能从
#'春','夏','秋','冬','unknow'中选一个,注意每次只能加一个
